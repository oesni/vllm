...
class SolarOpenMoE(nn.Module):
    def __init__(
        ...
        # See: https://github.com/huggingface/transformers/blob/v5.0.0/src/transformers/models/solar_open/modeling_solar_open.py
        self.gate = nn.Linear(
            config.hidden_size,
...
class SolarOpenDecoderLayer(nn.Module):
    def __init__(
        ...
        # Solar Open uses MoE (with one shared expert) on every layer:
        # unlike e.g. Glm4Moe, the architecture has no leading dense layers.
...
class SolarOpenMixtureOfExperts(MixtureOfExperts):
    def extract_moe_parameters(self, example_moe: SolarOpenMoE | None) -> None:
...
class SolarOpenForCausalLM(
    nn.Module, SupportsPP, SupportsLoRA, SolarOpenMixtureOfExperts
):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Solar Open uses MoE on every decoder layer.
        self.num_moe_layers = config.num_hidden_layers
        self.num_expert_groups = config.n_group

        self.moe_layers = []
        self.moe_mlp_layers: list[SolarOpenMoE] = []
